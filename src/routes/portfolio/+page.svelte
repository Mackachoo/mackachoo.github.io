<script lang="ts">
  ...
  function randomMargin(random: number): string {
    return ["p-2 mr-3 lg:p-8", "p-4 mr-5 lg:p-12", "p-6 ml-1 lg:p-14", "p-8 mr-2 lg:p-16", "p-10 ml-4 lg:p-20"][
      Math.floor(random * 100) % 5
    ];
  }

  function intersect(element: HTMLElement, callback: (isVisible: boolean) => void) {
...
{#each projects as project, index}
  ...
  <div
    ...
    class={"flex justify-start items-center flex-col " + String(side ? "lg:flex-row" : "lg:flex-row-reverse")}
    use:intersect={(isVisible) => {
      project.open = isVisible;
      projects = projects;
    }}
  >
    <!-- Moon component -->
    <div
      class={"w-full group md:w-[65%] z-10 p-10 orbit " +
        String(luminosity(project.colour) > 0.5 ? "text-black " : "text-white ") +
        ...
        >
          ...
          {#if !project.open}
            <div class="w-full sm:w-fit sm:mx-4 space-y-5" in:blur>
              <h1 class={project.title.length > 30 ? "text-lg sm:text-2xl" : "text-xl sm:text-2xl"}>
                {project.title}
              </h1>
            ...
            <div in:blur class="flex w-full justify-center">
              <img
                class={"opacity-70 w-24 sm:w-36 " + String(project.title.length > 20 ? "hidden sm:block" : "")}
                src={project.logo}
                alt={project.title}
              />
            </div>
          ...
          {:else}
            <img in:blur class="opacity-90 p-4" src={project.logo} alt={project.title} />
          {/if}
    ...
    <FlyingDescription {side} open={project.open}>
      <h1 class={(side ? "" : "lg:text-right") + " text-tan font-bold mb-4"}>
        {project.title}
      </h1>
      <div class={(side ? "lg:text-left" : "lg:text-right") + " text-justify"}>
        {project.desc}
      </div>
      {#if project.link}
        <div class={"flex mt-3 justify-end " + String(side ? "lg:justify-start" : "")}>
